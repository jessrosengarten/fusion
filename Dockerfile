...
WORKDIR /src
COPY ["TicketStatusAPI.csproj", "."]
RUN dotnet restore "./TicketStatusAPI.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./TicketStatusAPI.csproj" -c %BUILD_CONFIGURATION% -o /app/build

FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./TicketStatusAPI.csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=true

FROM mcr.microsoft.com/dotnet/runtime:8.0-nanoserver-1809 AS final
WORKDIR /app
EXPOSE 8080
COPY --from=publish /app/publish .
ENTRYPOINT ["TicketStatusAPI.exe"]